...
import {Logger, LodestarError} from "@lodestar/utils";

// Light client specific error types (simplified from beacon-node)
...
// Simplified attestation error for light client
export type AttestationErrorType = {code: string};
...
export type GossipModules = {
  ...
  libp2p: Libp2p;
  logger: Logger;
  // Note: Light client doesn't have a full beacon chain
};
...
// Note: Light client doesn't use job queues for gossip validation
